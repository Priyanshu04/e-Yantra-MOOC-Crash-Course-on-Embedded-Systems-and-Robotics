/*! \mainpage Section-4 Experiment-1: Interrupt Switch 
 *
...
void interrupt_sw_config(void) {
	...
	EIMSK_reg |=(1<< interrupt_switch_pin);
	
	// Low level detection on INT7
	EICRB_reg &=~ (1<< interrupt_ISC_switch_bit0) ;
	EICRB_reg &=~ (1<< interrupt_ISC_switch_bit1);
